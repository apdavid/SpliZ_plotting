...
def load_gtf(gtf_file,filt_chr):
  ...
  try:
    ...
    try:
      ...
      gtf["gene_id"] = gtf["attribute"].str.split("ID").str[1].str.split(";").str[0].str.split('"').str[1]
...
def main():
  ...
  for gene in tqdm(genes):
      ...
      for let in ["Start", "End"]:
        ...
        print("gene_df size",gene_df.shape)
        print(gene_df[["juncStart","juncEnd"]])
        suff = ""
        ...
        print(gene_df[["juncStart","juncEnd","num_" + let_dict[let],"pos{}_group".format(let),"pos{}_group".format(rev_dict[let])]])

        # only plot for those with more than one splice partner
        gene_df = gene_df[gene_df["num_" + let_dict[let]] > 1]
        print("gene size 1",gene_df.shape)
        ...
        box(gene_df, let, cell_lim, gene, args.dataname, let_dict, palette, rev_dict, domains, gtf, alpha, comp_sort, suff, args.grouping_level_1, args.grouping_level_2,rankLabel,args.outpath)
